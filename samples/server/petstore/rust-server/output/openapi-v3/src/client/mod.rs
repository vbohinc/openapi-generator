    ...
    F: Future<Item=Response<Body>, Error=hyper::Error> + Send + 'static
{
    ...
    fn required_octet_stream_put(&self, param_body: swagger::ByteArray, context: &C) -> Box<Future<Item=RequiredOctetStreamPutResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            ...
            match response.status().as_u16() {
                200 => {
                    let response_success_info = match response.headers().get(HeaderName::from_static("success-info")) {
                        ...
                        None => return Box::new(future::err(ApiError(String::from("Required response header Success-Info for response 200 was not found.")))) as Box<Future<Item=_, Error=_> + Send>,
                    };
                    let body = response.into_body();
                    Box::new(
                        body
                ...
                412 => {
                    let response_further_info = match response.headers().get(HeaderName::from_static("further-info")) {
                        ...
                        None => return Box::new(future::err(ApiError(String::from("Required response header Further-Info for response 412 was not found.")))) as Box<Future<Item=_, Error=_> + Send>,
                    };
                    let response_failure_info = match response.headers().get(HeaderName::from_static("failure-info")) {
                        ...
                        None => return Box::new(future::err(ApiError(String::from("Required response header Failure-Info for response 412 was not found.")))) as Box<Future<Item=_, Error=_> + Send>,
                    };
                    let body = response.into_body();
                    Box::new(
                        future::ok(
    ...
    fn xml_extra_post(&self, param_duplicate_xml_object: Option<models::DuplicateXmlObject>, context: &C) -> Box<Future<Item=XmlExtraPostResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/xml_extra",
            self.base_path
        );
    ...
    fn xml_other_post(&self, param_another_xml_object: Option<models::AnotherXmlObject>, context: &C) -> Box<Future<Item=XmlOtherPostResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/xml_other",
            self.base_path
        );
    ...
    fn xml_other_put(&self, param_string: Option<models::AnotherXmlArray>, context: &C) -> Box<Future<Item=XmlOtherPutResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/xml_other",
            self.base_path
        );
    ...
    fn xml_post(&self, param_string: Option<models::XmlArray>, context: &C) -> Box<Future<Item=XmlPostResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/xml",
            self.base_path
        );
    ...
    fn xml_put(&self, param_xml_object: Option<models::XmlObject>, context: &C) -> Box<Future<Item=XmlPutResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/xml",
            self.base_path
        );
