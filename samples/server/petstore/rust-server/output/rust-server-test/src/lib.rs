...
pub trait ApiNoContext {
    ...
    /// A dummy endpoint to make the spec valid.
    fn dummy_get(&self) -> Box<Future<Item=DummyGetResponse, Error=ApiError> + Send>;


    fn dummy_put(&self, nested_response: models::InlineObject) -> Box<Future<Item=DummyPutResponse, Error=ApiError> + Send>;

    /// Get a file
    fn file_response_get(&self) -> Box<Future<Item=FileResponseGetResponse, Error=ApiError> + Send>;

    /// Test HTML handling
    fn html_post(&self, body: String) -> Box<Future<Item=HtmlPostResponse, Error=ApiError> + Send>;

    /// Get an arbitrary JSON blob.
    fn raw_json_get(&self) -> Box<Future<Item=RawJsonGetResponse, Error=ApiError> + Send>;
