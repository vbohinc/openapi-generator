    ...
    F: Future<Item=Response<Body>, Error=hyper::Error> + Send + 'static
{
    ...
    fn dummy_put(&self, param_nested_response: models::InlineObject, context: &C) -> Box<Future<Item=DummyPutResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/dummy",
            self.base_path
        );
    ...
    fn html_post(&self, param_body: String, context: &C) -> Box<Future<Item=HtmlPostResponse, Error=ApiError> + Send> {
        let mut uri = format!(
            "{}/html",
            self.base_path
        );
